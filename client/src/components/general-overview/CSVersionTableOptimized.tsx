...
import { CSVersionDashboardOptimized } from './CSVersionDashboardOptimized';
...
interface CSVersionTableOptimizedProps {
  dateRange?: { start: string | null; end: string | null };
  onDateRangeChange?: (start: string | null, end: string | null) => void;
}

const COOKIE_KEYS = {
  SEARCH: 'CS_VERSION_SEARCH',
  SORT_FIELD: 'CS_VERSION_SORT_FIELD',
  SORT_ORDER: 'CS_VERSION_SORT_ORDER',
  VISIBLE_COLUMNS: 'CS_VERSION_VISIBLE_COLUMNS'
};

export function CSVersionTableOptimized({ 
  dateRange = { start: null, end: null },
  onDateRangeChange
}: CSVersionTableOptimizedProps) {
  const { theme } = useTheme();
  const [aggregatedData, setAggregatedData] = useState<AggregatedCSVersionEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState(cookies.get(COOKIE_KEYS.SEARCH) || '');
  const [summarySortField, setSummarySortField] = useState<keyof AggregatedCSVersionEntry>(
    (cookies.get(COOKIE_KEYS.SORT_FIELD) as keyof AggregatedCSVersionEntry) || 'newestSN'
  );
  ...
  const fetchAggregatedData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/general-overview/cs-version?${params.toString()}`,
        {
      ...
      if (response.ok) {
        ...
        showToast('Failed to fetch CS version data', 'error');
      }
    } catch (error) {
      console.error('Error fetching CS version data:', error);
      showToast('Error fetching CS version data', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSummarySort = (field: keyof AggregatedCSVersionEntry) => {
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <CSVersionDashboardOptimized dateRange={dateRange} />
