...
import { CSVersionDashboard } from './CSVersionDashboard';
...
interface AggregatedCSVersionEntry {
  csVersion: string;
...
interface CSVersionTableProps {
  selectedSNs?: string[];
  dateRange?: { start: string | null; end: string | null };
}

export function CSVersionTable({ 
  selectedSNs = [],
  ...
  const [summarySortField, setSummarySortField] = useState<'csVersion' | 'totalFlightTime' | 'oldestLogDate' | 'newestLogDate' | 'newestSN'>('newestSN');
  const [summarySortOrder, setSummarySortOrder] = useState<'asc' | 'desc'>('asc');
  const [selectedCSVersions, setSelectedCSVersions] = useState<string[]>([]);
  const [csVersionDateRange, setCSVersionDateRange] = useState<{ start: string | null; end: string | null }>({ 
    start: null, 
    end: null 
  });
  ...
  // Aggregate CS version data directly from filtered log entries
  const aggregatedCSVersions = useMemo((): AggregatedCSVersionEntry[] => {
    const csVersionMap = new Map<string, AggregatedCSVersionEntry>();
    
    filteredLogEntries.forEach(entry => {
      // Only process entries with valid CS version
      if (entry.cs_version && entry.cs_version.trim() !== '') {
        const csVersion = entry.cs_version;
        if (csVersionMap.has(csVersion)) {
          const existing = csVersionMap.get(csVersion)!;
    ...
    return Array.from(csVersionMap.values());
  }, [filteredLogEntries]);

  // Get unique CS versions for filter
  const uniqueCSVersions = useMemo(() => {
    const versions = new Set(aggregatedCSVersions.map(e => e.csVersion));
    return Array.from(versions).sort();
  }, [aggregatedCSVersions]);

  const csVersionOptions = useMemo(() => 
    uniqueCSVersions.map(version => ({ value: version, label: version }))
  , [uniqueCSVersions]);

  // Filter and sort aggregated CS versions for the summary table
  const filteredAndSortedAggregated = useMemo(() => {
    return aggregatedCSVersions
      .filter(entry => {
        const matchesSearch = entry.csVersion.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        const matchesCSVersion = selectedCSVersions.length === 0 || selectedCSVersions.includes(entry.csVersion);
        return matchesSearch && matchesCSVersion;
      })
      .sort((a, b) => {
        let aVal: any = a[summarySortField as keyof AggregatedCSVersionEntry];
        let bVal: any = b[summarySortField as keyof AggregatedCSVersionEntry];
  ...
  }, [aggregatedCSVersions, searchQuery, summarySortField, summarySortOrder, selectedCSVersions]);

  const handleSummarySort = (field: 'csVersion' | 'totalFlightTime' | 'oldestLogDate' | 'newestLogDate' | 'newestSN') => {
  ...
  const getExportFilename = () => {
    let filename = 'cs_version_overview';
    const parts = [];
    if (selectedCSVersions.length > 0) {
      parts.push(`CS_${selectedCSVersions.join('_')}`);
    }
    if (csVersionDateRange.start || csVersionDateRange.end) {
      const start = csVersionDateRange.start || 'start';
      const end = csVersionDateRange.end || 'end';
  ...
  const getSheetTitle = () => {
    ...
    const parts = [`CS Version Overview - Report Generated: ${reportDate}`];
    
    if (selectedCSVersions.length > 0) {
      parts.push(`CS Version: ${selectedCSVersions.join(', ')}`);
    }
    if (csVersionDateRange.start || csVersionDateRange.end) {
      const start = csVersionDateRange.start || 'Start';
      const end = csVersionDateRange.end || 'End';
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      {selectedSNs.length === 0 && !dateRange.start && !dateRange.end && (
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-start gap-2">
            <div className="w-48">
              <MultiSelect
                value={selectedCSVersions}
                onChange={setSelectedCSVersions}
                options={csVersionOptions}
                placeholder="All CS Versions"
              />
            </div>
            <div className="w-48">
              <DateRangePicker
                key={`${csVersionDateRange.start}-${csVersionDateRange.end}`}
                onApply={(start, end) => setCSVersionDateRange({ start, end })}
                onCancel={() => {}}
                initialStart={csVersionDateRange.start}
                initialEnd={csVersionDateRange.end}
              />
            </div>
            {(selectedCSVersions.length > 0 || csVersionDateRange.start !== null) && (
              <button
                onClick={() => {
                  setSelectedCSVersions([]);
                  setCSVersionDateRange({ start: null, end: null });
                }}
      ...
      <CSVersionDashboard 
        ...
        dateRange={dateRange}
        selectedCSVersions={selectedCSVersions}
        csVersionDateRange={csVersionDateRange}
      />

      {/* Search and Actions */}
      <div className="flex justify-between items-center gap-2">
        <h3 className={`text-sm font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Summary by CS Version</h3>
            ...
            sheets={[
              {
                ...
                columns: [
                  ...
                  { key: 'CS Version', label: 'CS Version' },
        ...
        <table className="w-full text-xs">
          <thead className={`text-xs uppercase sticky top-0 z-10 ${
            ...
            <tr>
              ...
              {visibleColumns.csVersion && (
                <th 
                  onClick={() => handleSummarySort('csVersion')}
                  className="px-3 py-2 cursor-pointer hover:bg-gray-600 transition text-left"
                >
                  <div className="flex items-center justify-start gap-1">
                    CS Version
                    {summarySortField === 'csVersion' && (
                      <span>{summarySortOrder === 'asc' ? '↑' : '↓'}</span>
                    )}
                  </div>
                </th>
          ...
          <tbody className={theme === 'dark' ? 'text-white' : 'text-gray-900'}>
            {filteredAndSortedAggregated.length > 0 ? (
              filteredAndSortedAggregated.map((entry, idx) => (
                <tr key={entry.csVersion} className={`border-b transition ${
                  ...
                  {visibleColumns.csVersion && <td className="px-3 py-2 font-medium text-left">{entry.csVersion}</td>}
      ...
      <div className={`text-xs text-right ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
        Showing {filteredAndSortedAggregated.length} unique CS versions
      </div>
    </div>
  );
}
