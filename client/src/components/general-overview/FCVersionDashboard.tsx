...
interface FCVersionDashboardProps {
  selectedSNs?: string[];
  dateRange?: { start: string | null; end: string | null };
  selectedFCVersions?: string[];
  fcVersionDateRange?: { start: string | null; end: string | null };
}

export function FCVersionDashboard({
  selectedSNs = [],
  dateRange = { start: null, end: null },
  selectedFCVersions = [],
  fcVersionDateRange = { start: null, end: null }
}: FCVersionDashboardProps) {
  ...
  const filteredLogEntries = useMemo(() => {
    ...
    return logEntries.filter(entry => {
      ...
      // Only include entries with valid FC version
      const hasValidFCVersion = entry.fc_version && entry.fc_version.trim() !== '';
      
      return matchesSN && matchesDate && hasValidFCVersion;
    });
  }, [logEntries, selectedSNs, dateRange, fcVersionDateRange]);

  // Filter by FC versions if provided
  const finalEntries = useMemo(() => {
    if (selectedFCVersions.length === 0) return filteredLogEntries;
    return filteredLogEntries.filter(entry => selectedFCVersions.includes(entry.fc_version));
  }, [filteredLogEntries, selectedFCVersions]);

  const stats = useMemo(() => {
    const uniqueFCVersions = new Set(finalEntries.map(e => e.fc_version)).size;
