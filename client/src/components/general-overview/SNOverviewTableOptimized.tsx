...
import { SNOverviewDashboardOptimized } from './SNOverviewDashboardOptimized';
...
export function SNOverviewTableOptimized() {
  ...
  // Fetch detailed data when filters are applied or "See All Data" is clicked
  useEffect(() => {
    if (showAllData || selectedSNs.length > 0 || dateRange.start || dateRange.end) {
  ...
  }, [showAllData, selectedSNs, dateRange]);
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      // Fetch aggregated data from new endpoint
      const response = await fetch(
        `${API_BASE_URL}/general-overview/sn-overview`,
        {
  ...
  const fetchDetailedEntries = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast('Failed to fetch detailed entries', 'error');
      }
    } catch (error) {
      console.error('Error fetching detailed entries:', error);
      showToast('Error fetching detailed entries', 'error');
    } finally {
  ...
  const filteredLogEntries = useMemo(() => {
    // Use detailed entries if available, otherwise use aggregated entries
    const entriesToFilter = detailedEntries.length > 0 ? detailedEntries : [];
  ...
  }, [detailedEntries, selectedSNs, dateRange]);

  // Aggregate filtered log entries by SN (only when we have detailed data)
  const aggregatedEntries = useMemo((): AggregatedSNEntry[] => {
    // If no filters and no detailed data, use the aggregated entries from server
    if (!hasActiveFilters && detailedEntries.length === 0) {
      return logEntries.map(entry => ({
        sn: entry.sn,
        ulogFiles: entry.ulogFiles,
        totalFlightTime: entry.totalFlightTime,
        lastUsage: entry.lastUsage,
      }));
    }

    // Otherwise aggregate from filtered detailed entries
  ...
  }, [logEntries, filteredLogEntries, hasActiveFilters, detailedEntries]);
  ...
  // Filter and sort aggregated entries
  const filteredAndSortedAggregated = useMemo(() => {
    return aggregatedEntries
      .filter(entry => entry.sn.toLowerCase().includes(searchQuery.toLowerCase()))
      .sort((a, b) => {
        let aVal: any;
        let bVal: any;
  ...
  // Filter and sort individual log entries
  ...
  const summaryTotals = useMemo(() => {
    if (hasActiveFilters && detailedEntries.length > 0) {
      const uniqueSNs = new Set(filteredAndSortedDetailed.map((e: DetailedLogEntry) => e.sn)).size;
      const totalFiles = filteredAndSortedDetailed.length;
      const totalFlightTime = filteredAndSortedDetailed.reduce((sum: number, e: DetailedLogEntry) => sum + e.flight_time, 0);
      const totalDistance = filteredAndSortedDetailed.reduce((sum: number, e: DetailedLogEntry) => sum + (e.distance || 0), 0);
      return { uniqueSNs, totalFiles, totalFlightTime, totalDistance };
    } else {
      ...
      const totalDistance = logEntries.reduce((sum: number, e: LogEntry) => sum + (e.totalDistance || 0), 0);
      return { uniqueSNs, totalFiles, totalFlightTime, totalDistance };
    }
  }, [hasActiveFilters, filteredAndSortedDetailed, filteredAndSortedAggregated, logEntries, detailedEntries]);
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      <SNOverviewDashboardOptimized 
        selectedSNs={selectedSNs}
        dateRange={dateRange}
      />

      {/* Flight Hours Chart - Only show when detailed data is loaded */}
      ...
      {/* Show loading indicator when fetching detailed data */}
      {loadingDetailed && (
        <div className={`text-center py-4 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
          <p className="text-sm">Loading detailed data...</p>
        </div>
      )}
