...
export function MultiSelect({
  ...
  return (
    <div ref={containerRef} className={`relative ${className} ${isOpen ? 'z-[9999]' : ''}`}>
      ...
      >
        <span className={`truncate ${value.length === 0 ? (theme === 'dark' ? 'text-white' : 'text-gray-500') : ''}`}>
          {getDisplayText()}
        </span>
        <ChevronDown className={`h-3 w-3 transition-transform flex-shrink-0 ml-2 ${isOpen ? 'rotate-180' : ''} ${
